...
/** 新增菜单节点 */
export class PageNodeDtoCreate implements Omit<PageNode, 'id'|'parentId'|'icon'|'hasSchema'|'isDeleted'> {
    @ApiProperty(model2ApiProperty('name', {action: 'create'}))
    name!: PageNode['name'];
      
    @ApiProperty(model2ApiProperty('nameCh', {action: 'create'}))
    nameCh!: PageNode['nameCh'];
    ...
    @ApiProperty(model2ApiProperty('icon', {action: 'create'}))
    icon?: PageNode['icon'];
...
export class PageNodeDtoUpdate {
    ...
    @ApiProperty(model2ApiProperty('icon'))
    icon?: PageNode['icon'];
